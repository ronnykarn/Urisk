...
def riskToUtility(county: str, county_data: dict, samples):

    risk = np.empty(0)
    for netMetring in [True, False]:
        ...
        risk = np.append(risk, avgProjectedLoss / (np.multiply(normResUnit.loadProfile, normResUnit.retailPrice).sum()))
